...
def compute_query_features(opt, model, testset, test_queries):
    ...
    mods = []
    for t in tqdm(test_queries):
        imgs += [testset.get_img(t['source_img_id'], raw_img=False)]
        mods += [t['mod']['str']]
        if len(imgs) >= opt.batch_size or t is test_queries[-1]:
            if 'torch' not in str(type(imgs[0])):
                imgs = [torch.from_numpy(d).float() for d in imgs]
...
def inference(opt, model, test_queries, dataset, category=None):
    model.eval()
    all_imgs = []
    all_captions = []
    all_queries = []
    ...
    for t in test_queries:
        imgs += [dataset.get_img(t['source_img_id'])]
        mods += [t['mod']['str']]
        if len(imgs) >= opt.batch_size or t is test_queries[-1]:
            if 'torch' not in str(type(imgs[0])):
                    imgs = [torch.from_numpy(d).float() for d in imgs]
        ...
        all_queries = np.concatenate(all_queries)
        all_imgs, all_captions = \
            compute_db_features(opt, model, dataset, category)
            
    nn_result, sorted_sims = nn_and_sims(dataset, all_queries, all_imgs,
                                        test_queries, all_captions,
                                        category=category)
    ...
    #         os.makedirs(dirname)
    #     fname = os.path.join(dirname, "{}.predict.json".format(category))
    #     write_fashioniq(nn_result, dataset, test_queries,
    #                     sorted_sims, fname, category)
